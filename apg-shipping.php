...
//Carga el idioma
load_plugin_textdomain( 'apg_shipping', null, dirname( plugin_basename( __FILE__ ) ) . '/i18n/languages' );
...
function apg_shipping_enlace_de_ajustes( $enlaces ) { 
	...
	$enlaces_de_ajustes = array(
		'<a href="' . $apg_shipping['ajustes'] . '" title="' . __( 'Settings of ', 'apg_shipping' ) . $apg_shipping['plugin'] .'">' . __( 'Settings', 'apg_shipping' ) . '</a>', '<a href="' . $apg_shipping['soporte'] . '" title="' . __( 'Support of ', 'apg_shipping' ) . $apg_shipping['plugin'] .'">' . __( 'Support', 'apg_shipping' ) . '</a>'
	);
	...
	class apg_shipping extends WC_Shipping_Method {				
		...
		function calculate_shipping( $paquete = array() ) {
			...
			foreach ( WC()->cart->get_cart() as $identificador => $valores ) {
				...
				//Medidas
				if ( $producto->length ) {
					$largo += ( $producto->length * $valores['quantity'] );
				}
				if ( $producto->width ) {
					$ancho += ( $producto->width * $valores['quantity'] );
				}
				if ( $producto->height ) {
					$alto += ( $producto->height * $valores['quantity'] );
				}
			...
			//Cargos adicionales
			if ( $this->fee > 0 ) {
				$precio_total += $this->fee;			
			}
			if ( $this->cargo > 0 && !strpos( $this->cargo, '%' ) ) {
				$precio_total += $this->cargo;
			}

			foreach ( $precios as $grupo => $precio ) {
				if ( $this->cargo > 0 && strpos( $this->cargo, '%' ) ) {
					$precio += $precio * ( str_replace( '%', '', $this->cargo ) / 100 ); //Cargos adicionales
				}
				$precio_total += $precio;
				if ( $this->tax_status != 'none' ) {
					$impuestos_parciales[$grupo] = $impuestos->calc_shipping_tax( $precio_total, $impuestos->get_shipping_tax_rates( $this->settings['Tax_' . $grupo] ) );
				}
			}
		...
		function dame_tarifa_mas_barata( $tarifas, $peso_total, $largo, $ancho, $alto, $grupos, $clases ) {
			...
			if ( !empty( $grupos ) ) {
				foreach ( $grupos as $clase => $grupo ) {
					...
					foreach ( $tarifas as $indice => $tarifa ) {	
						if ( strpos( $tarifa[2], $grupo ) !== false ) { //El grupo existe en las tarifas recogidas.
							...
							if ( isset( $medidas ) ) { //Son medidas
								...
								if ( $largo > $medidas[0] || $ancho > $medidas[1] || $alto > $medidas[2] ) {
									$tamano = true;
								}
							}
							...
							if ( !$dimensiones && !$tamano ) { //Es un peso
								...
								if ( !$largo_anterior || ( ( $medidas[0] >= $largo && $largo > $largo_anterior ) && ( $medidas[1] >= $ancho && $ancho > $ancho_anterior ) && ( $medidas[2] >= $alto && $alto > $alto_anterior ) ) ) {
									$gasto_de_envio[$grupo] = $tarifa[1];
					...
					if ( $this->maximo == "no" && ( $peso_parcial > $peso_anterior ) ) {
		...
		function dame_paises_especificos() {
			...
			if ( is_array( $paises_especificos_permitidos) ) {
				$paises = array_merge( $paises_nuevos, $paises_especificos_permitidos );
			}
