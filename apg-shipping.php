...
$apg_shipping = array('plugin' => 'WooCommerce - APG Weight and Postcode/State/Country Shipping', 
						...
						'ajustes' => 'admin.php?page=woocommerce_settings&tab=shipping&section=apg_shipping', 
						'imagen' => 'http://www.artprojectgroup.es/wp-content/artprojectgroup/woocommerce-apg-weight-and-postcodestatecountry-shipping-582x139.jpg', 
...
function apg_shipping_enlace_de_ajustes($enlaces) { 
	...
	$enlace_de_ajustes = '<a href="' . $apg_shipping['ajustes'] . '" title="' . __('Settings of ', 'apg_shipping') . $apg_shipping['plugin'] . '">' . __('Settings', 'apg_shipping') . '</a>'; 
...
function apg_shipping_inicio() {
	...
	class apg_shipping extends WC_Shipping_Method {				
		...
		function calculate_shipping($paquete = array()) {
			...
			if ($this->apg_free_shipping && $this->muestra == 'yes') return false;
...
function dame_medios_de_pago() {
	global $woocommerce;
	
	$medios = array();	
	
	if (!empty($woocommerce->payment_gateways)) 
	{
		$payment_gateways = $woocommerce->payment_gateways;
		foreach ($payment_gateways as $medios_de_pago)
		{
			foreach ($medios_de_pago as $medio_de_pago)
			{
				$medio_de_pago = get_object_vars($medio_de_pago);
				if ($medio_de_pago['enabled'] == 'yes') $medios[$medio_de_pago['id']] = $medio_de_pago['title'];
			}
		}
	}
...
function filtra_medios_de_pago($medios){
	global $woocommerce;
...
add_filter('woocommerce_available_payment_gateways', 'filtra_medios_de_pago');
